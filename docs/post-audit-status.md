...
| C-04 | Deposits without on-chain verification | **By design** | Same as C-02: single-operator with shell = game over. SPV proof inside enclave protects against malicious operator, not compromised server (attacker bypasses orchestrator). Real fix: multi-operator — 2-of-3 independently monitor XRPL ledger and confirm deposits. One rogue operator cannot credit fake deposits. |
