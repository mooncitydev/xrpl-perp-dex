...
- **It's not immortal.** Intel has removed SGX from newer consumer CPUs (laptops, desktops). Server SGX on Xeon chips is still supported and is what we target. Intel's newer **TDX** (Trust Domain Extensions) is a related technology that protects whole VMs instead of just processes — it's meant to complement, not replace, server SGX for now, and it's a separate topic.
...
5. **Intel has deprecated consumer SGX.** Don't try to run this on a laptop. Server-grade Xeon only.
